...
class Laporan extends CI_Controller
{
    ...
    public function laundry()
    {
        ...
        $this->template->load('template/template', 'laporan/laundry_list', $data);
    ...
    public function transaksi()
    {
        ...
        $this->template->load('template/template', 'laporan/transaksi_list', $data);
        $this->load->view('template/datatables', $data);
    }
    ...
    public function berobat()
    {
        ...
        $this->template->load('template/template', 'laporan/berobat_list', $data);
        $this->load->view('template/datatables', $data);
    }
    public function kas_galon()
    {
        if (!$this->ion_auth->is_admin()) {
            show_error('Hanya Administrator yang diberi hak untuk mengakses halaman ini, <a href="' . base_url('dashboard') . '">Kembali ke menu awal</a>', 403, 'Akses Dilarang!');
        }
        ...
        $this->template->load('template/template', 'laporan/kas_galon_list', $data);
        $this->load->view('template/datatables', $data);
    }
    public function cukur()
    {
        if (!$this->ion_auth->is_admin()) {
            show_error('Hanya Administrator yang diberi hak untuk mengakses halaman ini, <a href="' . base_url('dashboard') . '">Kembali ke menu awal</a>', 403, 'Akses Dilarang!');
        }
    ...
    public function transaksidata()
    {
        // $id = $this->input->post('id');
        $id = 'laundry';
        $this->output_json($this->Keuangan_model->getDataKeperluan2(), false);
    }

    public function berobatdata()
    {
        // $id = $this->input->post('id');
        $id = 'berobat';
        $this->output_json($this->Keuangan_model->getDataKeperluan($id), false);
